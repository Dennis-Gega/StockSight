...
int main() {
    ...
    server.Get("/api/prices", [&](const httplib::Request& req, httplib::Response& res){
        ...
        try {
            ...
            std::cout << "start_expr=" << start_expr << "\n";
            std::cout << "end_expr="   << end_expr   << "\n";
            ...
            std::cerr << "[ERROR] /api/prices: " << e.what() << std::endl;
            res.status = 500;
    ...
    // GET /api/indicators (dynamic range)
        ...
        try {
            ...
            if (n >= 2 &&
                ...
                if (!std::isnan(lastRsi)) {
                    if (lastRsi < 30.0)          score += 3.0;
                    else if (lastRsi > 70.0)     score -= 3.0;
                    else if (lastRsi < 40.0)     score += 1.0;
                    else if (lastRsi > 60.0)     score -= 1.0;

                    if (!std::isnan(prevRsi)) {
                        double delta = lastRsi - prevRsi;
                        if (delta > 1.0)          score += 1.0;
                        else if (delta < -1.0)    score -= 1.0;
                    }
                }
                ...
                if (!std::isnan(lastMacd) && !std::isnan(lastSignal)) {
                    ...
                    if (bullCross)      score += 3.0;
                    else if (bearCross) score -= 3.0;
                    else {
                        if (lastMacd > lastSignal)      score += 1.0;
                        else if (lastMacd < lastSignal) score -= 1.0;
                    }
                }
                ...
                if (!std::isnan(lastClose) &&
                    ...
                    if (range > 0) {
                        ...
                        else if (pos < 0.25)  score += 1.0;
                        else if (pos > 0.9)   score -= 2.0;
                        else if (pos > 0.75)  score -= 1.0;
                    }
                }

                // final
                if (score >= 1.0)       signal = "Buy";
            ...
            std::cerr << "[ERROR] /api/indicators: " << e.what() << std::endl;
            res.status = 500;
            res.set_content(std::string("{\"error\":\"") + e.what() + "\"}", "application/json");
        }
    });
