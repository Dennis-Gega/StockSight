...
std::vector<double> ema(const std::vector<double>& x, int n, bool doubleEMA) {
...
std::vector<double> rsi(const std::vector<double>& c, int n, bool wilder) {
    ...
    for (size_t i = n; i < c.size(); ++i) {
        if (i > static_cast<size_t>(n)) {
            ...
            if (wilder) {
                ...
                // Standard RSI smoothing (same formula here – you can tweak later)
                avgG = (avgG * (n - 1) + g) / n;
                avgL = (avgL * (n - 1) + l) / n;
            }
        }
...
MACD macd(const std::vector<double>& c, int fast, int slow, int sig, bool doubleEMA) {
    MACD m;
    if (c.empty()) return m;
...
std::vector<int> trendDirection(const std::vector<double>& series, int window) {
    // +1 = uptrend, -1 = downtrend, 0 = flat
    std::vector<int> trend(series.size(), 0);
    if (window <= 0 || series.size() <= static_cast<size_t>(window)) {
        return trend;
    }
...
std::vector<double> momentum(const std::vector<double>& series, int window) {
...
BB bollinger(const std::vector<double>& c, int period, double k) {
    ...
    if (n < static_cast<size_t>(period) || period <= 0) return b;
