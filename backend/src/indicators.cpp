...
MACD macd(const std::vector<double>& c, int fast, int slow, int sig){
  ...
  for (size_t i=0;i<n;++i){
    if (!std::isnan(emaF[i]) && !std::isnan(emaS[i])) m.macd[i] = emaF[i]-emaS[i];
  }
  m.signal = ema(m.macd, sig);
  m.hist.resize(n, NAN);
  for (size_t i=0;i<n;++i){
    if (!std::isnan(m.macd[i]) && !std::isnan(m.signal[i])) m.hist[i]=m.macd[i]-m.signal[i];
  }
  return m;
}

BB bollinger(const std::vector<double>& c, int period, double k){
  BB b; size_t n=c.size();
  b.upper.resize(n, NAN); b.middle.resize(n, NAN); b.lower.resize(n, NAN);
  if (n<period) return b;

  for (size_t i=period-1;i<n;++i){
    double sum=0, sumsq=0;
    for (int j=0;j<period;++j){
      double v=c[i-j]; sum+=v; sumsq+=v*v;
    }
